...
export default function Register() {
  ...
  const onSubmit = async (e) => {
    ...
    try {
      ...
      await register({ name: name.trim(), email: email.trim(), password, preferredLanguage: lang });
      navigate('/');
    } catch (err) {
  ...
  return (
    <div className="relative mx-auto flex min-h-screen max-w-lg flex-col justify-center gap-6 px-4 py-8">
      ...
      <div className="relative animate-fade-up space-y-3">
        <h1 className="font-display text-4xl font-extrabold leading-tight text-white">{t('register')}</h1>
